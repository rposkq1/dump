...
// Pad or truncate digits array to fixed length (pad with zeros at start)
int *pad_or_truncate_digits(int *digits, int original_len, int fixed_len) {
    int *padded = malloc(fixed_len * sizeof(int));
    int pad_count = fixed_len - original_len;

    if (pad_count < 0) {
        // truncate leading digits
        memcpy(padded, digits + (-pad_count), fixed_len * sizeof(int));
    } else {
        // pad leading zeros
        for (int i = 0; i < pad_count; i++) {
            padded[i] = 0;
        }
        memcpy(padded + pad_count, digits, original_len * sizeof(int));
    }
    return padded;
}
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        fprintf(stderr, "Usage: %s mode [inputfile] [outputfile] [fixed_length]\n", argv[0]);
    ...
    int fixed_length = 0;
    if (argc >= 5) {
        fixed_length = atoi(argv[4]);
        if (fixed_length <= 0) {
            fprintf(stderr, "Invalid fixed_length value.\n");
            return EXIT_FAILURE;
        }
    }
    ...
    if (mode == 1) { // Decimal to Factoradic
        mpz_t number;
        mpz_init(number);
        ...
        if (fixed_length > 0) {
            int *padded = pad_or_truncate_digits(digits, len, fixed_length);
            free(digits);
            digits = padded;
            len = fixed_length;
        }
    ...
    } else if (mode == 2) { // Factoradic to Decimal
        int *digits = NULL;
    ...
    } else if (mode == 3) { // Factoradic to Permutation
        int *digits = NULL;
        int len = read_digits_input(fin, &digits);
        ...
        if (fixed_length > 0) {
            if (len > fixed_length) {
                fprintf(stderr, "Error: Input factoradic digits length %d greater than fixed length %d\n", len, fixed_length);
                free(digits);
                if (fin) fclose(fin);
                if (fout != stdout) fclose(fout);
                return EXIT_FAILURE;
    ...
    } else if (mode == 4) { // Permutation to Factoradic
        int *perm = NULL;
        ...
        if (fixed_length > 0) {
            if (len != fixed_length) {
                fprintf(stderr, "Error: Permutation input length %d must equal fixed length %d\n", len, fixed_length);
                free(perm);
                if (fin) fclose(fin);
                if (fout != stdout) fclose(fout);
                return EXIT_FAILURE;
